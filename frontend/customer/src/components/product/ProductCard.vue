	...
	>
		...
		>
			...
			<div class="product-middle">
				<h3>{{ product.name }}</h3>
				<p class="price">{{ format.formatPrice(product.price) }}₫</p>
			</div>

			<div class="product-bottom">
				<button class="add-cart-btn" @click.prevent="handleAddToCart">
					<i class="fa-solid fa-cart-plus"></i>
				</button>
				<button class="buy-now-btn" @click.prevent="handleBuy">
					Mua ngay
				</button>
			</div>
		</div>
...
.buy-now-btn {
	...
	color: #fff;
	cursor: pointer;
	box-shadow: 0 4px 12px rgba(248, 110, 211, 0.2);
